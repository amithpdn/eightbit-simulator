...
from .serializers import InstructionSetSerializer, ExampleProgramSerializer, SimulatorSessionSerializer
import re 
...
class InstructionSetViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    serializer_class = InstructionSetSerializer    # Serializer for converting to/from JSON
...
class ExampleProgramViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    serializer_class = ExampleProgramSerializer    # Serializer for converting to/from JSON
...
class SimulatorSessionViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = SimulatorSessionSerializer  # Serializer for converting to/from JSON
    ...
    def create(self, request):
        ...
        if x_forwarded_for:
            ip = x_forwarded_for.split(',')[0]   # Extract first IP if multiple IPs in header
        else:
