...
const EightBitSimulator = () => {
  ...
  const [cpuState, setCpuState] = useState({
    ...
    activeComponents: {
      // Tracks active components for visualization
      ...
      try {
        ...
        setInstructionSet([
          { opcode: "0000", name: "NOP", description: "No Operation" },
          {
            opcode: "0001",
            name: "LDA",
            description: "Load value from memory address into A register",
          },
          {
            opcode: "0010",
            name: "ADD",
            description: "Add value from memory address to A register",
          },
          //{
          //  opcode: "0011",
          //  name: "SUB",
          //  description: "Subtract value from memory address from A register",
          //},
          //{
          //  opcode: "0100",
          //  name: "STA",
          //  description: "Store A register to memory address",
          //},
          //{
          //  opcode: "0101",
          //  name: "LDI",
          //  description: "Load immediate value into A register",
          //},
          //{
          //  opcode: "0110",
          //  name: "JMP",
          ...
          //  opcode: "1001",
          //  name: "OUT",
          //  description: "Output Register A to the output register",
          //},
          //{
          //  opcode: "1111",
          //  name: "HLT",
  ...
  const highlightAssembly = (code) => {
    if (!code)
      return [
        <div key="empty">
          Enter your assembly code here or select an example code to load...
        </div>,
      ];
          ...
          style={{
            ...
            backgroundColor:
              lineIndex === currentLine
                ? "rgba(0, 240, 255, 0.3)"
                : "transparent", // Light indigo background
            display: "flex",
            textAlign: "left",
            whiteSpace: "pre",
            fontFamily: "monospace",
            lineHeight: "1.5",
            fontSize: isMobile ? "10px" : "12px", // Smaller font on mobile
          }}
        >
          {/* Line number */}
          <div
  ...
  return (
    <div className="flex flex-col w-full min-h-screen pt-2 px-2 md:pt-4 md:px-4 bg-gray-50">
      <div className="m-1 md:m-2">
        {/* Controls Section - Responsive */}
        <div className="flex flex-wrap gap-2 mt-1 p-2 md:p-3 bg-white rounded-lg shadow-sm">
          {/* Examples List dropdown - Smaller on mobile */}
          <div className="flex items-center">
            <label htmlFor="ex_prog" className="hidden">
              Select Example Program:
            </label>
            <select
              id="ex_prog"
              className={`border p-1 md:p-2 text-xs md:text-base ${isRunning
                  ? "bg-indigo-300 text-indigo-100 cursor-not-allowed"
                  : "bg-indigo-600 text-white hover:bg-indigo-700"
                } rounded-l focus:outline-none focus:ring-2 focus:ring-indigo-500`}
            ...
            <button
              className={`p-1 md:p-2 rounded-r text-xs md:text-base ${!selectedExample || isRunning
          ...
          {/* Execute Code Button - Smaller on mobile */}
          <button
            className={`p-1 md:p-2 px-2 md:px-4 rounded shadow-sm text-xs md:text-base ${isRunning || !assemblyCode.trim()
                ? "bg-indigo-300 text-indigo-100 cursor-not-allowed"
          ...
          >
            {isMobile ? "Run" : "Execute Code"}
          </button>

          {/* Reset Button - Smaller on mobile */}
          <button
            className="bg-red-600 text-white p-1 md:p-2 px-2 md:px-4 rounded shadow-sm hover:bg-red-700 focus:ring-2 focus:ring-offset-2 focus:ring-red-500 text-xs md:text-base"
            onClick={() => {
              // Reset the CPU
          ...
          {/* Clock Speed Control - Condensed on mobile */}
          <div className="flex items-center">
            <span className="bg-indigo-600 text-white p-1 md:p-2 rounded-l shadow-sm flex items-center text-xs md:text-base">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-3 w-3 md:h-5 md:w-5 mr-1"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
              ...
              {isMobile ? "Speed" : "Clock Speed"}
            </span>
            <label htmlFor="clk_speed" className="hidden">
              Clock Speed:
            </label>
            <select
              id="clk_speed"
              className="border-0 p-1 md:p-2 bg-amber-500 hover:bg-amber-600 text-white rounded-r shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-opacity-50 text-xs md:text-base"
              value={clockSpeed}
              onChange={(e) => setClockSpeed(parseFloat(e.target.value))}
            >
              {/* Available clock speeds in Hz */}
              {[0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000].map(
                (speed) => (
          ...
          {/* Check Instruction Set Button - Smaller on mobile */}
          <button
            className="bg-emerald-600 text-white p-1 md:p-2 rounded shadow-sm hover:bg-emerald-700 focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 text-xs md:text-base"
            onClick={() => setShowInstructionSet(true)}
          >
            {isMobile ? "Instr." : "Instruction Set"}
          </button>
        </div>

        {/* Instruction Set Modal - Responsive */}
        {showInstructionSet && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2">
            <div className="bg-white p-3 md:p-6 rounded-lg shadow-xl w-full md:w-2/3 max-h-[90vh] overflow-auto">
              <h2 className="text-lg md:text-xl font-bold mb-2 md:mb-4 text-indigo-900">
                Instruction Set
              </h2>
                ...
                <table className="w-full border-collapse text-xs md:text-base">
                  <thead>
                    <tr className="bg-indigo-100">
                      <th className="p-1 md:p-2 border border-indigo-200">
                        Opcode
                      </th>
                      <th className="p-1 md:p-2 border border-indigo-200">
                        Instruction
                  ...
                  <tbody>
                    {instructionSet.map((instr, idx) => (
                      ...
                      >
                        <td className="p-1 md:p-2 text-center border border-indigo-100 font-mono">
                          {instr.opcode}
                        </td>
                        <td className="p-1 md:p-2 text-center border border-indigo-100 font-mono font-semibold text-indigo-700">
                          {instr.name}
                        </td>
                        <td className="p-1 md:p-2 border border-indigo-100">
                          {instr.description}
                        </td>
                      </tr>
                    ))}
              ...
              <button
                className="mt-3 md:mt-4 bg-indigo-600 text-white p-1 md:p-2 px-2 md:px-4 rounded hover:bg-indigo-700 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-xs md:text-base"
      ...
      {/* Main simulator layout - Responsive grid/flex */}
      <div className={`${isPortrait ? 'flex flex-col' : 'grid grid-cols-1 md:grid-cols-3'} 
  ${isPortrait ? 'gap-4' : 'gap-2 md:gap-6'} mb-3 md:mb-6 overflow-y-auto`}>
        {/* Assembly Code Editor Column */}
        <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 p-2 md:p-4 rounded-lg shadow-lg flex flex-col"
          style={{
            height: isPortrait ? (isMobile ? '280px' : '25vh') : 'auto',
            marginBottom: isPortrait && isMobile ? '16px' : '0'
          }}>
          <h2 className="text-white text-sm md:text-xl mb-1 md:mb-3 text-center font-bold">
          ...
          {isEditing ? (
            ...
            <div className="flex-grow w-full bg-white rounded-md shadow-inner overflow-hidden relative">
              <div className="flex h-full">
                {/* Line numbers */}
                <div
                  className="bg-gray-50 border-r border-gray-200 text-gray-500 font-mono text-right pr-1 md:pr-2 pt-2 md:pt-4 select-none"
                  style={{
                    minWidth: "2em",
                    overflowY: "hidden",
                ...
                <textarea
                  ...
                  className="font-mono flex-grow bg-white overflow-auto p-2 md:p-4 pl-1 md:pl-2"
                  value={assemblyCode}
          ...
          {/* Code editor controls - Responsive */}
          <div className="mt-1 md:mt-3 flex justify-between">
            <div>
              {isEditing ? (
                <button
                  className={`p-1 md:p-2 rounded text-xs md:text-base ${isRunning
            ...
            <div>
              <button
                className={`p-1 md:p-2 rounded mr-1 md:mr-2 text-xs md:text-base ${isRunning || !assemblyCode.trim()
                    ? "bg-gray-500 text-gray-100 cursor-not-allowed"
                    : "bg-gray-800 text-white hover:bg-gray-700 focus:ring-2 focus:ring-gray-500 px-2 md:px-4"
                  }`}
              ...
              <button
                className={`p-1 md:p-2 rounded text-xs md:text-base ${isRunning || !assemblyCode.trim()
        ...
        <div className="flex flex-col gap-2 md:gap-6"
          style={{
            height: isPortrait ? (isMobile ? '320px' : '25vh') : 'auto',
            marginBottom: isPortrait && isMobile ? '16px' : '0'
          }}>

          {/* ROM Content */}
          <div className="bg-gradient-to-br from-purple-600 to-purple-700 p-2 md:p-4 rounded-lg shadow-lg flex flex-col flex-1">
            <h2 className="text-white text-sm md:text-xl mb-1 md:mb-3 text-center font-bold">
              ROM Memory
            </h2>
            <div className="bg-white rounded-md shadow-inner flex-grow p-1 md:p-3 font-mono text-xs overflow-auto">
              <div className="grid grid-cols-9 gap-0.5 md:gap-1">
                {/* Add row headers */}
                <div className="flex items-center justify-center font-bold border-b-2 border-r-2 border-gray-200 text-gray-700 text-xs">
                  Addr
                </div>
                {Array.from({ length: 8 }, (_, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-center font-bold border-b-2 border-gray-200 text-gray-700 text-xs"
                  >
                ...
                {Array.from(
                  { length: isPortrait && isMobile ? 5 : 8 },
                  (_, row) => (
                    <React.Fragment key={row}>
                      {/* Row address label */}
                      <div className="flex items-center justify-center font-bold border-r-2 border-gray-200 text-gray-700 text-xs">
          ...
          {/* RAM Content */}
          <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 p-2 md:p-4 rounded-lg shadow-lg flex flex-col flex-1">
            <h2 className="text-white text-sm md:text-xl mb-1 md:mb-3 text-center font-bold">
              RAM Memory
            </h2>
            <div className="bg-white rounded-md shadow-inner flex-grow p-1 md:p-3 font-mono text-xs overflow-auto"
              style={{ padding: isMobile && isPortrait ? '2px' : '' }}>
              <div className="grid grid-cols-9 gap-0.5 md:gap-1">
                {/* Header row */}
                <div className="flex items-center justify-center font-bold border-b-2 border-r-2 border-gray-200 text-gray-700 text-xs">
                  Addr
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 p-2 md:p-4 rounded-lg shadow-lg flex flex-col"
          style={{ height: isPortrait ? (isMobile ? '300px' : '25vh') : 'auto' }}>
          <h2 className="text-white text-sm md:text-xl mb-1 md:mb-3 text-center font-bold">
            CPU Simulator
          </h2>
          <div className="bg-white rounded-md shadow-inner p-1 md:p-4 flex-grow flex flex-col">
            {/* CPU Flags Display */}
            <div className="bg-gray-50 p-1 md:p-3 rounded-md border border-gray-200 mb-2 md:mb-4 grid place-items-center">
              <p className="font-mono text-xs md:text-sm text-gray-700 flex items-center gap-1 md:gap-2">
                Flags:
                <span
                  className={`${cpuState.flags.zero
                      ? "bg-emerald-100 text-emerald-800 border border-emerald-300"
                      : "bg-gray-100 text-gray-500 border border-gray-300"
                    } py-0.5 px-1 rounded-md flex items-center font-mono text-xs`}
                >
                  <span
                    className={`inline-block w-1.5 h-1.5 rounded-full mr-1 ${cpuState.flags.zero ? "bg-emerald-500" : "bg-gray-400"
                      }`}
                  ></span>
                  Z: {cpuState.flags.zero ? "1" : "0"}
                </span>
                <span
                  className={`ml-1 md:ml-2 ${cpuState.flags.carry
                      ? "bg-amber-100 text-amber-800 border border-amber-300"
                      : "bg-gray-100 text-gray-500 border border-gray-300"
                    } py-0.5 px-1 rounded-md flex items-center font-mono text-xs`}
                >
                  <span
                    className={`inline-block w-1.5 h-1.5 rounded-full mr-1 ${cpuState.flags.carry ? "bg-amber-500" : "bg-gray-400"
                      }`}
                  ></span>
                  C: {cpuState.flags.carry ? "1" : "0"}
                </span>
              </p>
            </div>

            {/* Responsive CPU Visualization */}
            {isMobile && isPortrait ? (
              // Simplified visualization for mobile portrait mode
              <div className="flex-grow overflow-auto">
                <div className="grid grid-cols-2 gap-2 text-xs p-1">
                  <div
                    className={`border rounded p-1 ${cpuState.activeComponents.register_a
                  ...
                  >
                    PC: 0x{formatHex(cpuState.programCounter)}
                  </div>
                  <div
                    className={`border rounded p-1 ${cpuState.activeComponents.instruction_register
                        ? "bg-blue-100 text-blue-800 border-blue-400"
                        : "bg-gray-50 text-gray-700"
                      }`}
                  >
                    IR: 0x{formatHex(cpuState.instructionRegister)}
                  </div>
                  <div
                    className={`border rounded p-1 ${cpuState.activeComponents.alu
                        ? "bg-blue-100 text-blue-800 border-blue-400"
                        : "bg-gray-50 text-gray-700"
                      }`}
                  >
                    ALU: {cpuState.alu?.operation || "NONE"}
                  </div>
                  <div
                    className={`border rounded p-1 ${cpuState.activeComponents.output_register
                        ? "bg-amber-100 text-amber-800 border-amber-400"
                        : "bg-gray-50 text-gray-700"
                      }`}
                  >
                    Output: 0x{formatHex(cpuState.outputRegister)}
                  </div>
                </div>

                {/* Output value display block */}
                <div
                  className={`mt-2 p-1 rounded border ${cpuState.activeComponents.output_register
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-1 text-xs">
                    <div>Hex: 0x{formatHex(cpuState.outputRegister)}</div>
                    <div>Dec: {cpuState.outputRegister}</div>
                    <div>Bin: {formatBinary(cpuState.outputRegister)}</div>
                  </div>
                </div>
              </div>
            ) : (
              // Full SVG visualization for larger screens
              <div className="flex-grow">
                <svg viewBox="0 0 400 400" className="w-full h-full">
                  ...
                  <rect
                    ...
                    height="30"
                    fill={
                      cpuState.activeComponents.register_a
                        ? "#3B82F6"
                        : "#BFDBFE"
                    }
                    stroke="#1E40AF"
                  ...
                  <rect
                    ...
                    height="30"
                    fill={
                      cpuState.activeComponents.register_b
                        ? "#3B82F6"
                        : "#BFDBFE"
                    }
                    stroke="#1E40AF"
                  ...
                  <rect
                    ...
                    height="30"
                    fill={
                      cpuState.activeComponents.output_register
                        ? "#F59E0B"
                        : "#FEF3C7"
                    }
                    stroke="#B45309"
                  ...
                  <rect
                    ...
                    height="30"
                    fill={
                      cpuState.activeComponents.instruction_register
                        ? "#3B82F6"
                        : "#BFDBFE"
                    }
                    stroke="#1E40AF"
                  ...
                  <rect
                    ...
                    height="30"
                    fill={
                      cpuState.activeComponents.program_counter
                        ? "#3B82F6"
                        : "#BFDBFE"
                    }
                    stroke="#1E40AF"
                  ...
                  <rect
                    ...
                    width="100"
                    height="40"
                    fill={cpuState.activeComponents.rom ? "#8B5CF6" : "#DDD6FE"}
                    stroke="#7C3AED"
                    strokeWidth="2"
                    rx="3"
                  />
                  <text
                    ...
                    fontFamily="Courier New"
                    fontWeight="bold"
                    fill={cpuState.activeComponents.rom ? "white" : "#7C3AED"}
                  >
                    ROM
                  </text>
                  ...
                  <rect
                    ...
                    width="100"
                    height="40"
                    fill={cpuState.activeComponents.ram ? "#34D399" : "#A7F3D0"}
                    stroke="#10B981"
                    strokeWidth="2"
                    rx="3"
                  />
                  <text
                    ...
                    fontFamily="Courier New"
                    fontWeight="bold"
                    fill={cpuState.activeComponents.ram ? "white" : "#10B981"}
                  >
                    RAM
                  </text>
                  ...
                  <text
                    ...
                    fontFamily="Courier New"
                    fontWeight="bold"
                    fill={cpuState.activeComponents.alu ? "white" : "#1E40AF"}
                  >
                    ALU
                  </text>
                  ...
                  {cpuState.activeComponents.alu && (
                    <text
                      ...
                      fontWeight="bold"
                    >
                      Op:{cpuState.alu?.operation || "NONE"}
                    </text>
                  )}
                  ...
                  >
                    Hex:0x
                    <tspan fontWeight="bold" dx="2">
                      {formatHex(cpuState.outputRegister)}
                    </tspan>
                  </text>
                  {/* Decimal value */}
                  <text
                  ...
                  >
                    Bin:
                    <tspan fontWeight="bold" dx="2">
                      {formatBinary(cpuState.outputRegister)}
                    </tspan>
                  </text>
                </svg>
              </div>
      ...
      {/* Terms of Use Modal - Responsive */}
      {showTerms && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2">
          <div className="bg-white p-3 md:p-6 rounded-lg shadow-xl w-full md:w-3/4 max-h-[90vh] overflow-auto">
            <h2 className="text-lg md:text-xl font-bold mb-2 md:mb-4 text-indigo-900">
              Terms of Use
            </h2>
            ...
            <div className="mb-2 md:mb-4 text-xs md:text-base">
              ...
              <p className="text-gray-700">
                This 8-bit computer simulator is provided strictly for
                educational purposes. It is designed to help users understand
                basic computer architecture principles and assembly language
            ...
            <div className="mb-2 md:mb-4 text-xs md:text-base">
              ...
              <p className="text-gray-700">
                This simulator is an independent educational tool. The
                instruction set and architecture are simplified models designed
                for educational purposes and are not intended to infringe on any
                proprietary systems.
              </p>
            </div>
            ...
            <div className="mb-2 md:mb-4 text-xs md:text-base">
              ...
              <p className="text-gray-700">
                This simulator is provided "as is", without warranty of any
                kind. The creators are not responsible for any damage or issues
            ...
            <div className="mb-2 md:mb-4 text-xs md:text-base">
              ...
              <p className="text-gray-700">
                All code, examples, and content within this simulator are
                released under the MIT License. You are free to use, modify, and
                distribute this for educational purposes.
              </p>
            </div>

            <button
              className="mt-2 md:mt-4 bg-indigo-600 text-white px-2 md:px-4 py-1 md:py-2 rounded hover:bg-indigo-700 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-xs md:text-base"
      ...
      {/* First-time Disclaimer Modal - Responsive */}
      {!hasAcceptedDisclaimer && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2">
          <div className="bg-white p-3 md:p-6 rounded-lg shadow-xl max-w-full md:max-w-2xl">
            <h2 className="text-lg md:text-2xl font-bold mb-2 md:mb-4 text-indigo-900">
              Educational Purpose Disclaimer
            </h2>

            <div className="mb-3 md:mb-6 text-gray-700 text-xs md:text-base">
              <p className="mb-2 md:mb-3">
                Welcome to the 8-Bit Computer Simulator. This application is
                designed
                <strong> exclusively for educational purposes</strong> to help
                understand the basics of computer architecture and assembly
                programming.
              </p>

              <p className="mb-1 md:mb-3">Important notes:</p>

              <ul className="list-disc pl-4 md:pl-6 mb-2 md:mb-3">
              ...
              <p>
                By clicking "I Understand" below, you acknowledge that you are
                using this simulator for educational purposes only.
              </p>
            </div>

            <div className="flex justify-center">
              <button
                className="bg-indigo-600 text-white px-3 md:px-6 py-1 md:py-2 rounded hover:bg-indigo-700 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-xs md:text-base"
                onClick={() => {
      ...
      {/* Possible Improvements Modal - Responsive */}
      {showImprovements && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2">
          <div className="bg-white p-3 md:p-6 rounded-lg shadow-xl w-full md:w-3/4 max-h-[90vh] overflow-auto">
            <h2 className="text-lg md:text-xl font-bold mb-2 md:mb-4 text-indigo-900 mt-2 md:mt-5">
              Possible Improvements
            </h2>

            <div className="mb-3 md:mb-6 mx-2 md:mx-20 text-xs md:text-base">
              <p className="mb-2 md:mb-3 text-gray-700">
                The 8-bit computer simulator is a great educational tool, but
                there are many ways it could be enhanced. Here are some
                potential improvements:
              </p>

              <ul className="list-disc pl-4 md:pl-6 space-y-1 md:space-y-2 text-gray-700">
                <li className="p-1 hover:bg-indigo-50 rounded">
                  <span className="font-semibold text-indigo-700">
                    Add more instructions
                  </span>
                  : Implement more CPU instructions like bitwise operations,
                ...
                <li className="p-1 hover:bg-indigo-50 rounded">
                  ...
                  : Add a visual representation of memory access and data
                  movement.
                </li>
                ...
                <li className="p-1 hover:bg-indigo-50 rounded">
                  ...
                  : Implement simulated I/O devices like a display, keyboard
                  input, etc.
                </li>
                ...
                <li className="p-1 hover:bg-indigo-50 rounded">
                  ...
                  : Add breakpoints, step-by-step execution, and variable
                  inspection.
                </li>
              </ul>
            </div>

            <div className="bg-indigo-50 p-2 md:p-4 rounded-lg mb-2 md:mb-4 mx-2 md:mx-20 text-xs md:text-base">
              <p className="text-indigo-800">
                <span className="font-bold">Interested in contributing?</span>{" "}
                The source code for this simulator is available on{" "}
                <a
                  href="#"
      ...
      <div className="mt-2 md:mt-8 pt-2 md:pt-4 border-t border-gray-200 text-center text-gray-600 text-xs bg-white shadow-inner p-2 md:p-4 rounded-lg">
        <p>
          © {new Date().getFullYear()} 8-Bit Computer Simulator.{" "}
          <span className="hidden md:inline">All rights reserved.</span>
        </p>
        <p className="hidden md:block mt-1">
          This simulator is provided for educational purposes only and is not
          intended for commercial use.
        </p>
        <p className="mt-1 hidden md:block">
          This is an educational tool designed to teach basic computer
          architecture principles.
        </p>
        <p className="mt-1">
          <button
            className="text-indigo-600 hover:text-indigo-800 underline focus:outline-none text-xs"
